...
function getProxyEngine() {
    return localStorage.getItem('proxyEngine') || 'scramjet';
}

function setProxyEngine(engine) {
    localStorage.setItem('proxyEngine', 'scramjet');
    updateEngineUI();
    notify('success', 'Engine Ready', 'Using Scramjet Proxy Engine');
...
function updateEngineUI() {
    ...
    if (sjCard) sjCard.classList.add('active');
}

async function navigateTab(tab, targetUrl) {
    tab.url = targetUrl;
    tab.loading = true;
    ...
    try {
        const scramjet = await getSharedScramjet();
        if (scramjet && typeof scramjet.encodeUrl === 'function') {
            iframe.src = scramjet.encodeUrl(targetUrl);
        } else {
            iframe.src = getBasePath() + 'scramjet/' + encodeURIComponent(targetUrl);
        }
    } catch (e) {
        console.warn("Scramjet encode error:", e);
        iframe.src = getBasePath() + 'scramjet/' + encodeURIComponent(targetUrl);
    }
}
    ...
    iframe.addEventListener('load', () => {
        ...
        try {
            ...
            if (currentHref.includes('NT.html')) {
                ...
                tab.url = "";
                tab.favicon = null;
            } else if (sharedScramjet && typeof sharedScramjet.decodeUrl === 'function') {
                try {
                    const decoded = sharedScramjet.decodeUrl(currentHref);
                    if (decoded && (decoded.startsWith('http://') || decoded.startsWith('https://'))) {
                        ...
                        tab.title = win.document.title;
                    }
                } catch(e) {
                    if (win.document.title) tab.title = win.document.title;
                }
            } else if (currentHref.includes('/scramjet/')) {
                const parts = currentHref.split('/scramjet/');
...
document.addEventListener('DOMContentLoaded', async function () {
    try {
        ...
        // Keep address bar and tab title in sync for SPAs like YouTube
        setInterval(() => {
            const activeTab = getActiveTab();
            if (!activeTab || !activeTab.frame?.frame) return;
            try {
                const iframe = activeTab.frame.frame;
                const win = iframe.contentWindow;
                if (!win) return;
                const currentHref = win.location.href;
                if (!currentHref || currentHref.includes('NT.html') || currentHref === 'about:blank') return;
                
                if (sharedScramjet && typeof sharedScramjet.decodeUrl === 'function') {
                    const decoded = sharedScramjet.decodeUrl(currentHref);
                    if (decoded && decoded !== activeTab.url && (decoded.startsWith('http://') || decoded.startsWith('https://'))) {
                        activeTab.url = decoded;
                        if (win.document.title) activeTab.title = win.document.title;
                        updateTabsUI();
                        updateAddressBar();
                    }
                }
